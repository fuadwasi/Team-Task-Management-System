...
/// </summary>
public partial class MsSqlAssetForgeDataProvider : BaseDataProvider, IDataProvider
{
    #region Utilities
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task ResiteDatabaseAsync(string backupFileName)
    {
        using var currentConnection = CreateDataConnection();
    ...
    public virtual string BuildConnectionString(IConnectionStringInfo AssetForgeConnectionString)
    {
        ...
        {
            DataSource = AssetForgeConnectionString.ServerName,
            InitialCatalog = AssetForgeConnectionString.DatabaseName,
            PersistSecurityInfo = false,
            IntegratedSecurity = AssetForgeConnectionString.IntegratedSecurity,
            TrustServerCertificate = true
        };

        if (!AssetForgeConnectionString.IntegratedSecurity)
        {
            builder.UserID = AssetForgeConnectionString.Username;
            builder.Password = AssetForgeConnectionString.Password;
        }
    ...
    /// </summary>
    protected override global::LinqToDB.DataProvider.IDataProvider LinqToDbDataProvider => SqlServerTools.GetDataProvider(SqlServerVersion.v2012, SqlServerProvider.MicrosoftDataSqlClient);
