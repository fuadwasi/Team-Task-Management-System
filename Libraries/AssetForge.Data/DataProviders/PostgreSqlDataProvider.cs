...
using AssetForge.Data.DataProviders.LinqToDB;
using AssetForge.Data.Mapping;
using LinqToDB;
...
namespace AssetForge.Data.DataProviders;

public partial class PostgreSqlDataProvider : BaseDataProvider, IDataProvider
{
    #region Fields

    protected static readonly Lazy<global::LinqToDB.DataProvider.IDataProvider> _dataProvider = new(() => new LinqToDBPostgreSQLDataProvider(), true);
    ...
    protected virtual string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
    {
        ...
        var descriptor = MappingSchema.GetEntityDescriptor(typeof(TEntity))
                         ?? throw new Core.AssetForgeException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
        ...
        if (string.IsNullOrEmpty(columnName))
            throw new Core.AssetForgeException("A table's primary key does not have an identity constraint");
    ...
    public void CreateDatabase(string collation, int triesToConnect = 10)
    {
        ...
        {
            if (i == triesToConnect)
                throw new System.Exception("Unable to connect to the new database. Please try one more time");
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual Task ResiteDatabaseAsync(string backupFileName)
    {
        throw new DataException("This database provider does not support backup");
    }
    ...
    /// </summary>
    /// <param name="AssetForgeConnectionString">Connection string info</param>
    /// <returns>Connection string</returns>
    public virtual string BuildConnectionString(IConnectionStringInfo AssetForgeConnectionString)
    {
        ArgumentNullException.ThrowIfNull(AssetForgeConnectionString);

        if (AssetForgeConnectionString.IntegratedSecurity)
            throw new Core.AssetForgeException("Data provider supports connection only with login and password");

        var builder = new NpgsqlConnectionStringBuilder
        {
            Host = AssetForgeConnectionString.ServerName,
            //Cast DatabaseName to lowercase to avoid case-sensitivity problems
            Database = AssetForgeConnectionString.DatabaseName.ToLowerInvariant(),
            Username = AssetForgeConnectionString.Username,
            Password = AssetForgeConnectionString.Password,
        };
    ...
    {
        return $"AssetForge_{targetTable}_{targetColumn}";
    }
    ...
    protected override global::LinqToDB.DataProvider.IDataProvider LinqToDbDataProvider => _dataProvider.Value;
