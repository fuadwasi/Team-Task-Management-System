...
/// </summary>
public partial class MigrationAttribute : FluentMigrator.MigrationAttribute
{
    #region Fields
    ...
    /// <summary>
    /// Initializes a new instance of the AssetForgeMigrationAttribute class
    /// </summary>
    ...
    public MigrationAttribute(string dateTime, MigrationProcessType targetMigrationProcess = MigrationProcessType.NoMatter) :
    ...
    /// <param name="targetMigrationProcess">The target migration process</param>
    public MigrationAttribute(string dateTime, string description, MigrationProcessType targetMigrationProcess = MigrationProcessType.NoMatter) :
        this(new MigrationConfig
        {
            DateTime = dateTime,
    ...
    /// <param name="targetMigrationProcess">The target migration process</param>
    public MigrationAttribute(string dateTime, string AssetForgeVersion, UpdateMigrationType updateMigrationType, MigrationProcessType targetMigrationProcess = MigrationProcessType.NoMatter) :
        this(new MigrationConfig
        {
            DateTime = dateTime,
            AssetForgeVersion = AssetForgeVersion,
    ...
    protected MigrationAttribute(MigrationConfig config) : base(config.Version, config.Description)
    {
        _config = config;
    }
    ...
    protected partial class MigrationConfig
    {
        ...
        public virtual long Version
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(DateTime))
                    throw new Core.AssetForgeException("One of the following properties must be initialized: either Version or DateTime");

                var version = System.DateTime
                    .ParseExact(DateTime, MigrationDefaults.DateFormats, CultureInfo.InvariantCulture).Ticks;
        ...
        public virtual string Description
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(AssetForgeVersion))
                    throw new Core.AssetForgeException("One of the following properties must be initialized: either Description or AssetForgeVersion");

                if (UpdateMigrationType.HasValue)
                    description = string.Format(MigrationDefaults.UpdateMigrationDescription, AssetForgeVersion, UpdateMigrationType.ToString());
