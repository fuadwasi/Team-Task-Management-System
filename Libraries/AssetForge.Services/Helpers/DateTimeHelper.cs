...
public partial class DateTimeHelper : IDateTimeHelper
{
    ...
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the customer time zone; if customer is null, then default site time zone
    /// </returns>
    public virtual Task<TimeZoneInfo> GetCustomerTimeZoneAsync(Customer customer)
    {
        if (!_dateTimeSettings.AllowCustomersToSetTimeZone)
            return Task.FromResult(DefaultSiteTimeZone);
        ...
        return Task.FromResult(timeZoneInfo ?? DefaultSiteTimeZone);
    }
