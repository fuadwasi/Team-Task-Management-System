...
public partial class PluginService : IPluginService
{
    ...
    /// <summary>
    /// Check whether to load the plugin based on the site identifier passed
    /// </summary>
    /// <param name="pluginDescriptor">Plugin descriptor to check</param>
    /// <param name="siteId">Site identifier</param>
    /// <returns>Result of check</returns>
    protected virtual bool FilterBySite(PluginDescriptor pluginDescriptor, int siteId)
    {
        ArgumentNullException.ThrowIfNull(pluginDescriptor);
        ...
        if (!pluginDescriptor.LimitedToSites.Any())
            return true;

        return pluginDescriptor.LimitedToSites.Contains(siteId);
    }
    ...
    /// <param name="siteId">Filter by site; pass 0 to load all records</param>
    ...
    public virtual async Task<IList<PluginDescriptor>> GetPluginDescriptorsAsync<TPlugin>(LoadPluginsMode loadMode = LoadPluginsMode.InstalledOnly,
        Customer customer = null, int siteId = 0, string group = null, string dependsOnSystemName = "", string friendlyName = null, string author = null) where TPlugin : class, IPlugin
    {
        ...
        pluginDescriptors = await pluginDescriptors.WhereAwait(async descriptor =>
            ...
            FilterBySite(descriptor, siteId) &&
    ...
    public virtual async Task<PluginDescriptor> GetPluginDescriptorBySystemNameAsync<TPlugin>(string systemName,
        ...
        Customer customer = null, int siteId = 0, string @group = null) where TPlugin : class, IPlugin
    {
        return (await GetPluginDescriptorsAsync<TPlugin>(loadMode, customer, siteId, group))
    ...
    public virtual Task<string> GetPluginLogoUrlAsync(PluginDescriptor pluginDescriptor)
    {
        ...
        var logoPathUrl = _mediaSettings.UseAbsoluteImagePath ? _webHelper.GetSiteLocation() : $"{pathBase}/";
        ...
        if (checkDependencies)
        {
            ...
            if (descriptor.DependsOn?.Any() ?? false)
            {
                ...
                if (dependsOn.Any())
                {
                    ...
                    throw new Core.AssetForgeException(errorMessage);
                }
            }
        }
