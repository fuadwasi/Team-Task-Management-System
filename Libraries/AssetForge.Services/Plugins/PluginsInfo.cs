...
public partial class PluginsInfo : IPluginsInfo
{
    ...
    public PluginsInfo(IAssetForgeFileProvider fileProvider)
    {
    ...
    protected virtual IList<string> GetObsoleteInstalledPluginNames()
    {
        ...
        {
            //if not, try to parse the file that was used in previous ixCms versions
            filePath = _fileProvider.MapPath(PluginDefaults.ObsoleteInstalledPluginsFilePath);
            if (!_fileProvider.FileExists(filePath))
    ...
    protected IList<(string DescriptionFile, PluginDescriptor PluginDescriptor)> GetDescriptionFilesAndDescriptors(string directoryName)
    {
        ...
        //it's required: https://www.radiowavehub.store/boards/topic/17455/load-plugins-based-on-their-displayorder-on-startup
    ...
    public virtual void LoadPluginInfo()
    {
        ...
        foreach (var item in GetDescriptionFilesAndDescriptors(pluginsDirectory))
        {
            ...
            if (!pluginDescriptor.SupportedVersions.Contains(AssetForgeVersion.CURRENT_VERSION, StringComparer.InvariantCultureIgnoreCase))
            {
