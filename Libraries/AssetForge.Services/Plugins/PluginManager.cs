...
public partial class PluginManager<TPlugin> : IPluginManager<TPlugin> where TPlugin : class, IPlugin
{
    ...
    /// <summary>
    /// Prepare the dictionary key to site loaded plugins
    /// </summary>
    ...
    /// </returns>
    protected virtual async Task<string> GetKeyAsync(Customer customer, int siteId, string systemName = null)
    {
        return $"{siteId}-{(customer != null ? string.Join(',', await _customerService.GetCustomerRoleIdsAsync(customer)) : null)}-{systemName}";
    }
    ...
    /// </returns>
    protected virtual async Task<TPlugin> LoadPrimaryPluginAsync(string systemName, Customer customer = null, int siteId = 0)
    {
        //try to get a plugin by system name or return the first loaded one (it's necessary to have a primary active plugin)
        var plugin = await LoadPluginBySystemNameAsync(systemName, customer, siteId)
                     ?? (await LoadAllPluginsAsync(customer, siteId)).FirstOrDefault();
    ...
    /// </returns>
    public virtual async Task<IList<TPlugin>> LoadAllPluginsAsync(Customer customer = null, int siteId = 0)
    {
        //get plugins and put them into the dictionary to avoid further loading
        var key = await GetKeyAsync(customer, siteId);
        if (!_plugins.ContainsKey(key))
            _plugins.Add(key, await _pluginService.GetPluginsAsync<TPlugin>(customer: customer, siteId: siteId));
    ...
    /// The task result contains the plugin
    /// </returns>
    public virtual async Task<TPlugin> LoadPluginBySystemNameAsync(string systemName, Customer customer = null, int siteId = 0)
    {
        if (string.IsNullOrEmpty(systemName))
            return null;

        //try to get already loaded plugin
        var key = await GetKeyAsync(customer, siteId, systemName);
        if (_plugins.TryGetValue(key, out var value))
            return value.FirstOrDefault();

        //or get it from list of all loaded plugins or load it for the first time
        var pluginBySystemName = _plugins.TryGetValue(await GetKeyAsync(customer, siteId), out var plugins)
            ...
            ? loadedPlugin
            : (await _pluginService.GetPluginDescriptorBySystemNameAsync<TPlugin>(systemName, customer: customer, siteId: siteId))?.Instance<TPlugin>();
    ...
    /// </returns>
    public virtual async Task<IList<TPlugin>> LoadActivePluginsAsync(List<string> systemNames, Customer customer = null, int siteId = 0)
    {
        if (systemNames == null)
            return new List<TPlugin>();

        //get loaded plugins according to passed system names
        return (await LoadAllPluginsAsync(customer, siteId))
