...
namespace AssetForge.Services.Plugins.Marketplace;

/// <summary>
/// Represents the official feed manager (plugins from radiowavehub marketplace)
/// </summary>
public partial class OfficialFeedManager
{
    ...
    protected readonly Common.AssetForgeHttpClient _ixHttpClient;
    ...
    public OfficialFeedManager(ILogger logger,
        Common.AssetForgeHttpClient ixHttpClient)
    {
        _logger = logger;
    ...
    public virtual async Task<IList<OfficialFeedCategory>> GetCategoriesAsync()
    {
        ...
        try
        {
            xml.LoadXml(await _ixHttpClient.GetExtensionsCategoriesAsync());
        }
        catch (Exception ex)
        {
            await _logger.ErrorAsync("No access to the list of plugins. Website www.radiowavehub.store is not available.", ex);
        }
    ...
    public virtual async Task<IList<OfficialFeedVersion>> GetVersionsAsync()
    {
        ...
        try
        {
            xml.LoadXml(await _ixHttpClient.GetExtensionsVersionsAsync());
        }
        catch (Exception ex)
        {
            await _logger.ErrorAsync("No access to the list of plugins. Website www.radiowavehub.store is not available.", ex);
        ...
        int pageIndex = 0, int pageSize = int.MaxValue)
    {
        ...
        try
        {
            xml.LoadXml(await _ixHttpClient.GetExtensionsAsync(categoryId, versionId, price, searchTerm, pageIndex, pageSize));
        }
        catch (Exception ex)
        {
            await _logger.ErrorAsync("No access to the list of plugins. Website www.radiowavehub.store is not available.", ex);
