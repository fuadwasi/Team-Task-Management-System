...
namespace AssetForge.Services.Plugins;
...
public partial class UploadService : IUploadService
{
    ...
    public UploadService(IAssetForgeFileProvider fileProvider,
    ...
    protected virtual async Task<IList<UploadedItem>> GetUploadedItemsAsync(string archivePath)
    {
        ...
        var uploadedItemsFileEntry = archive.Entries
            .FirstOrDefault(entry => entry.Name.Equals(PluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
    ...
    protected virtual async Task<IDescriptor> UploadSingleItemAsync(string archivePath)
    {
        ...
        var pluginsDirectory = _fileProvider.MapPath(PluginDefaults.UploadedPath);
        ...
        if (!string.IsNullOrEmpty(PluginDefaults.ThemesPath))
            themesDirectory = _fileProvider.MapPath(PluginDefaults.ThemesPath);
        ...
        using (var archive = ZipFile.OpenRead(archivePath))
        {
            ...
            if (rootDirectories.Count != 1)
            {
                throw new Exception("The archive should contain only one root plugin or theme directory. " +
                                    ...
                                    $"To upload multiple items, the archive should have the '{PluginDefaults.UploadedItemsFileName}' file in the root");
            }
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                var isPluginDescriptor = entry.FullName
                    .Equals($"{uploadedItemDirectoryName}/{PluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                //or whether it's a theme descriptor
                var isThemeDescriptor = entry.FullName
                    .Equals($"{uploadedItemDirectoryName}/{PluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
                ...
                if (isPluginDescriptor)
                {
                    ...
                    if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(AssetForgeVersion.CURRENT_VERSION))
                        throw new Exception($"This plugin doesn't support the current version - {AssetForgeVersion.CURRENT_VERSION}");
                }
    ...
    protected virtual async Task<IList<IDescriptor>> UploadMultipleItemsAsync(string archivePath, IList<UploadedItem> uploadedItems)
    {
        ...
        using (var archive = ZipFile.OpenRead(archivePath))
        {
            foreach (var item in uploadedItems)
            {
                ...
                //ensure that the current version of ixCms is supported
                if (!item.SupportedVersions?.Contains(AssetForgeVersion.CURRENT_VERSION) ?? true)
                    continue;
                ...
                if (item.Type == UploadedItemType.Plugin)
                    descriptorPath = $"{itemPath}{PluginDefaults.DescriptionFileName}";

                if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(PluginDefaults.ThemeDescriptionFileName))
                    descriptorPath = $"{itemPath}{PluginDefaults.ThemeDescriptionFileName}";
                ...
                //ensure that the plugin current version is supported
                if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(AssetForgeVersion.CURRENT_VERSION))
                    continue;
    ...
    public virtual async Task<IList<IDescriptor>> UploadPluginsAndThemesAsync(IFormFile archivefile)
    {
        ...
        try
        {
            ...
            var tempDirectory = _fileProvider.MapPath(PluginDefaults.UploadsTempPath);
            ...
            //you can find a sample of such descriptive file in Libraries\AssetForge.Core\Plugins\Samples\
    ...
    public virtual async Task UploadIconsArchiveAsync(IFormFile archivefile)
    {
        ...
        try
        {
            ...
            //check if there is a folder for favicon and app icons for the current site (all site icons folders are in wwwroot/icons and are called icons_{siteId})
            var siteIconsPath = _fileProvider.GetAbsolutePath(string.Format(CommonDefaults.FaviconAndAppIconsPath, await _siteContext.GetActiveSiteScopeConfigurationAsync()));

            CreateDirectory(siteIconsPath);

            zipFilePath = _fileProvider.Combine(siteIconsPath, archivefile.FileName);
            await using (var fileStream = new FileStream(zipFilePath, FileMode.Create))
                await archivefile.CopyToAsync(fileStream);

            ZipFile.ExtractToDirectory(zipFilePath, siteIconsPath);
        }
        finally
        {
    ...
    public virtual async Task UploadFaviconAsync(IFormFile favicon)
    {
        ...
        //check if there is a folder for favicon (favicon folder is in wwwroot/icons and is called icons_{siteId})
        var siteFaviconPath = _fileProvider.GetAbsolutePath(string.Format(CommonDefaults.FaviconAndAppIconsPath, await _siteContext.GetActiveSiteScopeConfigurationAsync()));

        CreateDirectory(siteFaviconPath);

        var faviconPath = _fileProvider.Combine(siteFaviconPath, favicon.FileName);
    ...
    public virtual Task UploadLocalePatternAsync(CultureInfo cultureInfo = null)
    {
        ...
        try
        {
            ...
            var ziplocalePatternPath = getPath(CommonDefaults.LocalePatternPath, CommonDefaults.LocalePatternArchiveName);
            if (!_fileProvider.GetFileExtension(ziplocalePatternPath)?.Equals(".zip", StringComparison.InvariantCultureIgnoreCase) ?? true)
                throw new Exception($"Archive '{CommonDefaults.LocalePatternArchiveName}' to retrieve localization patterns not found.");
            ...
            if (!(checkDirectoryExists(CommonDefaults.LocalePatternPath, currentCulture.Name) ||
                  checkDirectoryExists(CommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
            {
                var cultureToUse = string.Empty;

                //3. Unpack the archive into a temporary folder
                ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(CommonDefaults.LocalePatternPath, tempFolder));

                //4. Search in the temp unpacked archive a folder with locales by culture
                var sourcelocalePath = _fileProvider.Combine(getPath(CommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
                if (_fileProvider.DirectoryExists(sourcelocalePath))
                {
                    cultureToUse = currentCulture.Name;
                }

                var sourcelocaleISOPath = _fileProvider.Combine(getPath(CommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
                ...
                {
                    var destlocalePath = getPath(CommonDefaults.LocalePatternPath, cultureToUse);
        ...
        finally
        {
            ...
            if (checkDirectoryExists(CommonDefaults.LocalePatternPath, tempFolder))
                _fileProvider.DeleteDirectory(getPath(CommonDefaults.LocalePatternPath, tempFolder));
        }
    ...
    public partial class UploadedItem
    {
        ...
        /// <summary>
        /// Gets or sets supported versions of ixCms
        /// </summary>
