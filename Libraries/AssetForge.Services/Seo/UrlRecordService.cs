...
namespace AssetForge.Services.Seo;
...
public partial class UrlRecordService : IUrlRecordService
{
    ...
    static UrlRecordService()
    {
        ...
        _seoCharacterTable = new Dictionary<char, string>
        {
            ...
            { '\u0184', "6" }, // LATIN CAPITAL LETTER TONE SAssetForge -- no decomposition
            { '\u0185', "6" }, // LATIN SMALL LETTER TONE SAssetForge -- no decomposition
    ...
    public virtual async Task<UrlRecord> GetBySlugAsync(string slug)
    {
        ...
        if (_localizationSettings.LoadAllUrlRecordsOnStartup)
        {
            var records = await _staticCacheManager.GetAsync(
                _staticCacheManager.PrepareKeyForDefaultCache(SeoDefaults.UrlRecordSlugLookupCacheKey),
        ...
        var key = _staticCacheManager.PrepareKeyForDefaultCache(SeoDefaults.UrlRecordBySlugCacheKey, slug);
    ...
    public virtual async Task<string> GetActiveSlugAsync(int entityId, string entityName, int languageId)
    {
        if (_localizationSettings.LoadAllUrlRecordsOnStartup)
        {
            ...
            var activeSlugs = await _staticCacheManager.GetAsync(
                _staticCacheManager.PrepareKeyForDefaultCache(SeoDefaults.UrlRecordEntityIdLookupCacheKey, languageId),
                async () => (await GetAllUrlRecordsAsync())
        ...
        //gradual loading
        var key = _staticCacheManager.PrepareKeyForDefaultCache(SeoDefaults.UrlRecordCacheKey, entityId, entityName, languageId);
