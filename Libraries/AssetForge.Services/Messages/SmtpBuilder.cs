...
public partial class SmtpBuilder : ISmtpBuilder
{
    ...
    protected virtual async Task<SaslMechanism> GetGmailCredentialsAsync(EmailAccount emailAccount)
    {
        ...
        if (string.IsNullOrEmpty(emailAccount.ClientId))
            throw new Core.AssetForgeException(await _localizationService.GetResourceAsync("Admin.Configuration.EmailAccounts.Fields.ClientId.Required"));

        if (string.IsNullOrEmpty(emailAccount.ClientSecret))
            throw new Core.AssetForgeException(await _localizationService.GetResourceAsync("Admin.Configuration.EmailAccounts.Fields.ClientSecret.Required"));

        var tokenFilePath = _fileProvider.MapPath(AssetForgeMessageDefaults.GmailAuthSitePath);
        ...
        var codeFlow = new GoogleAuthorizationCodeFlow(new GoogleAuthorizationCodeFlow.Initializer
        {
            ...
            Scopes = AssetForgeMessageDefaults.GmailScopes,
        ...
        if (authResult.Credential is null)
            throw new Core.AssetForgeException("Failed to obtain user credentials for the authorization server. Check the client secrets and allow the application to perform required operations.");
    ...
    protected virtual async Task<SaslMechanism> GetExchangeCredentialsAsync(EmailAccount emailAccount)
    {
        ...
        if (string.IsNullOrEmpty(emailAccount.TenantId))
            throw new Core.AssetForgeException(await _localizationService.GetResourceAsync("Admin.Configuration.EmailAccounts.Fields.TenantId.Required"));

        var confidentialClientApplication = ConfidentialClientApplicationBuilder.Create(emailAccount.ClientId)
            .WithAuthority(string.Format(AssetForgeMessageDefaults.MSALTenantPattern, emailAccount.TenantId))
            .WithClientSecret(emailAccount.ClientSecret)
            .Build();

        var authToken = await confidentialClientApplication.AcquireTokenForClient(AssetForgeMessageDefaults.MSALScopes).ExecuteAsync();
    ...
    public virtual async Task<SmtpClient> BuildAsync(EmailAccount emailAccount = null)
    {
        emailAccount ??= await _emailAccountService.GetEmailAccountByIdAsync(_emailAccountSettings.DefaultEmailAccountId)
                         ?? throw new Core.AssetForgeException("Email account could not be loaded");
        ...
        {
            client.Dispose();
            throw new Core.AssetForgeException(ex.Message, ex);
        }
    }
