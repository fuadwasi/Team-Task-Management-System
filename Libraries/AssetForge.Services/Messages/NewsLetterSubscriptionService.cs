...
public partial class NewsLetterSubscriptionService : INewsLetterSubscriptionService
{
    ...
    /// <summary>
    /// Gets a newsletter subscription by email and site ID
    /// </summary>
    ...
    /// The task result contains the newsLetter subscription
    /// </returns>
    public virtual async Task<NewsLetterSubscription> GetNewsLetterSubscriptionByEmailAndSiteIdAsync(string email, int siteId)
    {
    ...
    /// <param name="siteId">Site identifier. 0 to load all records.</param>
    ...
    public virtual async Task<IPagedList<NewsLetterSubscription>> GetAllNewsLetterSubscriptionsAsync(string email = null,
        ...
        int siteId = 0, bool? isActive = null, int customerRoleId = 0,
        int pageIndex = 0, int pageSize = int.MaxValue)
    {
        if (customerRoleId == 0)
        {
            ...
            var subscriptions = await _subscriptionRepository.GetAllPagedAsync(query =>
            {
                ...
                if (siteId > 0)
                    query = query.Where(nls => nls.SiteId == siteId);
        ...
        var guestRole = await _customerService.GetCustomerRoleBySystemNameAsync(CustomerDefaults.GuestsRoleName)
                        ?? throw new Core.AssetForgeException("'Guests' role could not be loaded");
        ...
        else
        {
            var subscriptions = await _subscriptionRepository.GetAllPagedAsync(query =>
            {
                ...
                if (siteId > 0)
                    joindQuery = joindQuery.Where(x => x.NewsletterSubscribers.SiteId == siteId);
                if (isActive.HasValue)
