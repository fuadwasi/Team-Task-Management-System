...
public partial class MessageTemplateService : IMessageTemplateService
{
    ...
    /// </returns>
    public virtual async Task<IList<MessageTemplate>> GetMessageTemplatesByNameAsync(string messageTemplateName, int? siteId = null)
    {
        if (string.IsNullOrWhiteSpace(messageTemplateName))
            throw new ArgumentException(nameof(messageTemplateName));

        var key = _staticCacheManager.PrepareKeyForDefaultCache(AssetForgeMessageDefaults.MessageTemplatesByNameCacheKey, messageTemplateName, siteId);
        ...
        return await _staticCacheManager.GetAsync(key, async () =>
        {
            ...
            if (siteId.HasValue && siteId.Value > 0)
                templates = await templates.WhereAwait(async messageTemplate => await _siteMappingService.AuthorizeAsync(messageTemplate, siteId.Value)).ToListAsync();
    ...
    /// </returns>
    public virtual async Task<IList<MessageTemplate>> GetAllMessageTemplatesAsync(int siteId, string keywords = null, bool? isActive = null)
    {
        ...
        }, cache => cache.PrepareKeyForDefaultCache(AssetForgeMessageDefaults.MessageTemplatesAllCacheKey, siteId, isActive));
    ...
    public virtual async Task<MessageTemplate> CopyMessageTemplateAsync(MessageTemplate messageTemplate)
    {
        ...
        //site mapping
        var selectedSiteIds = await _siteMappingService.GetSitesIdsWithAccessAsync(messageTemplate);
        foreach (var id in selectedSiteIds)
            await _siteMappingService.InsertSiteMappingAsync(mtCopy, id);
