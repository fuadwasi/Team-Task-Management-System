...
namespace AssetForge.Services.Localization
{
    ...
    public partial class LocalizationService : ILocalizationService
    {
        ...
        /// </summary>
        /// <param name="languageId">Language identifier</param>
        /// <param name="loadPublicLocales">A value indicating whether to load data for the public site only (if "false", then for admin area only. If null, then load all locales. We use it for performance optimization of the site startup</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the locale string resources
        /// </returns>
        ...
        public virtual async Task<string> ExportResourcesToXmlAsync(Language language)
        {
            ...
            await xmlWriter.WriteAttributeStringAsync("SupportedVersion", AssetForgeVersion.CURRENT_VERSION);
        ...
        public virtual async Task<string> GetLocalizedSettingAsync<TSettings>(TSettings settings, Expression<Func<TSettings, string>> keySelector,
            int languageId, int siteId, bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
            where TSettings : ISettings, new()
        {
            var key = _settingService.GetSettingKey(settings, keySelector);

            //we do not support localized settings per site (overridden site settings)
            var setting = await _settingService.GetSettingAsync(key, siteId: siteId, loadSharedValueIfNotFound: true);
            if (setting == null)
                return null;
