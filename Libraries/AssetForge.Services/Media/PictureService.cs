...
public partial class PictureService : IPictureService
{
    ...
    //protected readonly IWidgetAttributeParser _widgetAttributeParser;
    //protected readonly IWidgetAttributeService _widgetAttributeService;
    ...
    public PictureService(IDownloadService downloadService,
        ...
        //IWidgetAttributeParser widgetAttributeParser,
        //IWidgetAttributeService widgetAttributeService,
        ...
        MediaSettings mediaSettings)
    {
        ...
        //_widgetAttributeParser = widgetAttributeParser;
        //_widgetAttributeService = widgetAttributeService;
    ...
    /// </returns>
    protected virtual Task<string> GetImagesPathUrlAsync(string siteLocation = null)
    {
        var pathBase = _httpContextAccessor.HttpContext.Request?.PathBase.Value ?? string.Empty;
        var imagesPathUrl = _mediaSettings.UseAbsoluteImagePath ? siteLocation : $"{pathBase}/";
        imagesPathUrl = string.IsNullOrEmpty(imagesPathUrl) ? _webHelper.GetSiteLocation() : imagesPathUrl;
    ...
    /// The task result contains the local picture thumb path
    /// </returns>
    protected virtual async Task<string> GetThumbUrlAsync(string thumbFileName, string siteLocation = null)
    {
        var url = await GetImagesPathUrlAsync(siteLocation) + "thumbs/";
    ...
    {
        return await LoadPictureBinaryAsync(picture, await IsSiteInDbAsync());
    }
        ...
        string siteLocation = null)
    {
        ...
        if (defaultPictureType == PictureType.Entity && _mediaSettings.WidgetDefaultImageId > 0)
            return await GetPictureUrlAsync(_mediaSettings.WidgetDefaultImageId, targetSize, false, siteLocation, PictureType.Entity);
        ...
        if (targetSize == 0)
            return await GetImagesPathUrlAsync(siteLocation) + defaultImageFileName;
        ...
        if (!await GeneratedThumbExistsAsync(thumbFilePath, thumbFileName))
        {
            ...
            //we can't use semaphore here, because it produces PlatformNotSupportedException exception on UNAssetForge based systems
        ...
        return await GetThumbUrlAsync(thumbFileName, siteLocation);
    }
        ...
        PictureType defaultPictureType = PictureType.Entity)
    {
        ...
        return (await GetPictureUrlAsync(picture, targetSize, showDefaultPicture, siteLocation, defaultPictureType)).Url;
    }
    ...
    {
        if (picture == null)
            return showDefaultPicture ? (await GetDefaultPictureUrlAsync(targetSize, defaultPictureType, siteLocation), null) : (string.Empty, (Picture)null);
        ...
        if (picture.IsNew)
        {
            ...
            if ((pictureBinary?.Length ?? 0) == 0)
                return showDefaultPicture ? (await GetDefaultPictureUrlAsync(targetSize, defaultPictureType, siteLocation), picture) : (string.Empty, picture);
        ...
        if (targetSize == 0 || picture.MimeType == MimeTypes.ImageSvg)
        {
            var thumbFilePath = await GetThumbLocalPathAsync(thumbFileName);
            if (await GeneratedThumbExistsAsync(thumbFilePath, thumbFileName))
                return (await GetThumbUrlAsync(thumbFileName, siteLocation), picture);
        ...
        bool isNew = true, bool validateBinary = true)
    {
        ...
        await UpdatePictureBinaryAsync(picture, await IsSiteInDbAsync() ? pictureBinary : Array.Empty<byte>());
    ...
    public virtual async Task<Picture> UpdatePictureAsync(Picture picture)
    {
        ...
        await _pictureRepository.UpdateAsync(picture);
        await UpdatePictureBinaryAsync(picture, await IsSiteInDbAsync() ? (await GetPictureBinaryByPictureIdAsync(picture.Id)).BinaryData : Array.Empty<byte>());
    ...
    /// <summary>
    /// Get widget picture binary by picture identifier
    /// </summary>
    /// <param name="pictureId">The picture identifier</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    ...
    /// <summary>
    /// Get widget picture (for shopping cart and order details pages)
    /// </summary>
    ...
    /// The task result contains the picture
    /// </returns>
    //public virtual async Task<Picture> GetWidgetPictureAsync(Widget widget, string attributesXml)
    //{
    //    ArgumentNullException.ThrowIfNull(widget);

    //    first, try to get widget attribute combination picture
    //    var combination = await _widgetAttributeParser.FindWidgetAttributeCombinationAsync(widget, attributesXml);
    //    if (combination != null)
    //    {
    //        var combinationPicture = (await _widgetAttributeService.GetWidgetAttributeCombinationPicturesAsync(combination.Id)).FirstOrDefault();
    //        if (await GetPictureByIdAsync(combinationPicture?.PictureId ?? 0) is Picture picture)
    //            return picture;
    //    }

    //    then, let's see whether we have attribute values with pictures
    //    var values = await _widgetAttributeParser.ParseWidgetAttributeValuesAsync(attributesXml);
    //    foreach (var attributeValue in values)
    //    {
    //        var valuePictures = await _widgetAttributeService.GetWidgetAttributeValuePicturesAsync(attributeValue.Id);
    ...
    //    now let's load the default widget picture
    //    var widgetPicture = (await GetPicturesByWidgetIdAsync(widget.Id, 1)).FirstOrDefault();
    //    if (widgetPicture != null)
    //        return widgetPicture;

    //    finally, let's check whether this widget has some parent "grouped" widget
    //    if (widget.VisibleIndividually || widget.ParentGroupedWidgetId <= 0)
    //        return null;

    //    var parentGroupedWidgetPicture = (await GetPicturesByWidgetIdAsync(widget.ParentGroupedWidgetId, 1)).FirstOrDefault();
    //    return parentGroupedWidgetPicture;
    //}

    /// <summary>
    /// Gets a value indicating whether the images should be sited in data base.
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task<bool> IsSiteInDbAsync()
    {
        return await _settingService.GetSettingByKeyAsync("Media.Images.SiteInDB", true);
    }

    /// <summary>
    /// Sets a value indicating whether the images should be sited in data base
    /// </summary>
    /// <param name="isSiteInDb">A value indicating whether the images should be sited in data base</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task SetIsSiteInDbAsync(bool isSiteInDb)
    {
        //check whether it's a new value
        if (await IsSiteInDbAsync() == isSiteInDb)
            return;

        //save the new setting value
        await _settingService.SetSettingAsync("Media.Images.SiteInDB", isSiteInDb);
        ...
        try
        {
            while (true)
            {
                ...
                foreach (var picture in pictures)
                {
                    ...
                    var pictureBinary = await LoadPictureBinaryAsync(picture, !isSiteInDb);
                    ...
                    await UpdatePictureBinaryAsync(picture, isSiteInDb ? pictureBinary : Array.Empty<byte>());
                    picture.IsNew = true;
                }
