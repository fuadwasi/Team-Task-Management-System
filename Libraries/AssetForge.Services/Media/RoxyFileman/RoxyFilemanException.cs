...
public partial class RoxyFilemanException : Exception
{
    ...
    {
        var fileProvider = EngineContext.Current.Resolve<IAssetForgeFileProvider>();

        var roxyConfig = Singleton<RoxyFilemanConfig>.Instance;
        var languageFile = fileProvider.GetAbsolutePath($"{RoxyFilemanDefaults.LanguageDirectory}/{roxyConfig.LANG}.json");

        if (!fileProvider.FileExists(languageFile))
            languageFile = fileProvider.GetAbsolutePath($"{RoxyFilemanDefaults.LanguageDirectory}/en.json");
