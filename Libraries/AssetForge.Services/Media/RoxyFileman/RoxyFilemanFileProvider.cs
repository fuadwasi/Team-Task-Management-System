...
namespace AssetForge.Services.Media.RoxyFileman;
...
public partial class RoxyFilemanFileProvider : PhysicalFileProvider, IRoxyFilemanFileProvider
{
    ...
    protected Core.Infrastructure.IAssetForgeFileProvider _ixFileProvider;
    ...
    public RoxyFilemanFileProvider(Core.Infrastructure.IAssetForgeFileProvider ixFileProvider) : base(ixFileProvider.Combine(ixFileProvider.WebRootPath, RoxyFilemanDefaults.DefaultRootDirectory))
    {
        _ixFileProvider = ixFileProvider;
    }

    public RoxyFilemanFileProvider(Core.Infrastructure.IAssetForgeFileProvider defaultFileProvider, MediaSettings mediaSettings) : this(defaultFileProvider)
    {
    ...
    /// </summary>
    /// <param name="pathBase">The base path for the site</param>
    ...
    public virtual async Task<RoxyFilemanConfig> GetOrCreateConfigurationAsync(string pathBase, string lang)
    {
        ...
        if (Singleton<RoxyFilemanConfig>.Instance?.RETURN_URL_PREFAssetForge?.Equals(pathBase) ?? false)
        {
            return Singleton<RoxyFilemanConfig>.Instance;
        }

        var filePath = _ixFileProvider.GetAbsolutePath(RoxyFilemanDefaults.ConfigurationFile);

        //create file if not exists
        _ixFileProvider.CreateFile(filePath);

        //try to read existing configuration
        var existingText = await _ixFileProvider.ReadAllTextAsync(filePath, Encoding.UTF8);
        ...
        var configuration = new RoxyFilemanConfig
        {
            ...
            FORBIDDEN_UPLOADS = existingConfiguration?.FORBIDDEN_UPLOADS ?? string.Join(" ", RoxyFilemanDefaults.ForbiddenUploadExtensions),
            ...
            INTEGRATION = "custom",
            RETURN_URL_PREFAssetForge = $"{pathBase}/images/uploaded/",
