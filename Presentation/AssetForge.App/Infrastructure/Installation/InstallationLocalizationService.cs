...
namespace AssetForge.Web.Infrastructure.Installation;
        ...
        foreach (var filePath in _fileProvider.EnumerateFiles(_fileProvider.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
        {
            ...
            foreach (XmlNode resNode in resources)
            {
                ...
                if (resNameAttribute == null)
                    throw new Core.AssetForgeException("All installation resources must have an attribute Name=\"Value\".");
                var resourceName = resNameAttribute.Value.Trim();
                if (string.IsNullOrEmpty(resourceName))
                    throw new Core.AssetForgeException("All installation resource attributes 'Name' must have a value.'");

                if (resValueNode == null)
                    throw new Core.AssetForgeException("All installation resources must have an element \"Value\".");
