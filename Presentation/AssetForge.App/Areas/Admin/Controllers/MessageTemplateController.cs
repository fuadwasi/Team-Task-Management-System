...
public partial class MessageTemplateController : BaseAdminController
{
    ...
    protected readonly ISiteMappingService _storeMappingService;
    ...
    public MessageTemplateController(ICustomerActivityService customerActivityService,
        ...
        ISiteMappingService storeMappingService,
    ...
    protected virtual async Task SaveSiteMappingsAsync(MessageTemplate messageTemplate, MessageTemplateModel model)
    {
        messageTemplate.LimitedToSites = model.SelectedSiteIds.Any();
        await _messageTemplateService.UpdateMessageTemplateAsync(messageTemplate);

        var existingSiteMappings = await _storeMappingService.GetSiteMappingsAsync(messageTemplate);
        var allSites = await _storeService.GetAllSitesAsync();
        foreach (var store in allSites)
        {
            if (model.SelectedSiteIds.Contains(store.Id))
            {
                //new store
                if (!existingSiteMappings.Any(sm => sm.SiteId == store.Id))
                    await _storeMappingService.InsertSiteMappingAsync(messageTemplate, store.Id);
            }
            else
            {
                //remove store
                var storeMappingToDelete = existingSiteMappings.FirstOrDefault(sm => sm.SiteId == store.Id);
                if (storeMappingToDelete != null)
                    await _storeMappingService.DeleteSiteMappingAsync(storeMappingToDelete);
            }
        }
    }
    ...
    public virtual async Task<IActionResult> Edit(MessageTemplateModel model, bool continueEditing)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            //stores
            await SaveSiteMappingsAsync(messageTemplate, model);
