...
public partial class CountryController : BaseAdminController
{
    ...
    protected virtual async Task SaveSiteMappingsAsync(Country country, CountryModel model)
    {
        country.LimitedToSites = model.SelectedSiteIds.Any();
        await _countryService.UpdateCountryAsync(country);

        var existingSiteMappings = await _siteMappingService.GetSiteMappingsAsync(country);
        var allSites = await _siteService.GetAllSitesAsync();
        foreach (var site in allSites)
        {
            if (model.SelectedSiteIds.Contains(site.Id))
            {
                //new site
                if (!existingSiteMappings.Any(sm => sm.SiteId == site.Id))
                    await _siteMappingService.InsertSiteMappingAsync(country, site.Id);
            }
            else
            {
                //remove site
                var siteMappingToDelete = existingSiteMappings.FirstOrDefault(sm => sm.SiteId == site.Id);
                if (siteMappingToDelete != null)
                    await _siteMappingService.DeleteSiteMappingAsync(siteMappingToDelete);
            }
        }
    }
    ...
    public virtual async Task<IActionResult> Create(CountryModel model, bool continueEditing)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            //Sites
            await SaveSiteMappingsAsync(country, model);
    ...
    public virtual async Task<IActionResult> Delete(int id)
    {
        ...
        try
        {
            if (await _addressService.GetAddressTotalByCountryIdAsync(country.Id) > 0)
                throw new AssetForgeException("The country can't be deleted. It has associated addresses");
