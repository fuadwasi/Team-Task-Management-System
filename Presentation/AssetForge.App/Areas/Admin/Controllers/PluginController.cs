...
public partial class PluginController : BaseAdminController
{
    ...
    public virtual async Task<IActionResult> UploadPluginsAndThemes(IFormFile archivefile)
    {
        ...
        try
        {
            if (archivefile == null || archivefile.Length == 0)
                throw new AssetForgeException(await _localizationService.GetResourceAsync("Admin.Common.UploadFile"));
    ...
    public virtual async Task<IActionResult> EditPopup(PluginModel model)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            //we allow editing of 'friendly name', 'display order', site mappings
            pluginDescriptor.FriendlyName = model.FriendlyName;
            pluginDescriptor.DisplayOrder = model.DisplayOrder;
            pluginDescriptor.LimitedToSites.Clear();
            if (model.SelectedSiteIds.Any())
                pluginDescriptor.LimitedToSites = model.SelectedSiteIds;
