...
public partial class LanguageController : BaseAdminController
{
    ...
    protected virtual async Task SaveSiteMappingsAsync(Language language, LanguageModel model)
    {
        language.LimitedToSites = model.SelectedSiteIds.Any();
        await _languageService.UpdateLanguageAsync(language);

        var existingSiteMappings = await _siteMappingService.GetSiteMappingsAsync(language);
        ...
        foreach (var site in allSites)
        {
            if (model.SelectedSiteIds.Contains(site.Id))
            {
                ...
                if (!existingSiteMappings.Any(sm => sm.SiteId == site.Id))
                    await _siteMappingService.InsertSiteMappingAsync(language, site.Id);
            }
            else
            {
                //remove site
    ...
    public virtual async Task<IActionResult> Create(LanguageModel model, bool continueEditing)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            //Sites
            await SaveSiteMappingsAsync(language, model);
    ...
    //action displaying notification (warning) to a site owner that changed culture
