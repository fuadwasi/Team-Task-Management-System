    ...
    public virtual async Task<IActionResult> BackupAction(MaintenanceModel model)
    {
        ...
        try
        {
            switch (action)
            {
                ...
                case "resite-backup":
                    {
                        await _dataProvider.ResiteDatabaseAsync(backupPath);
                        _notificationService.SuccessNotification(await _localizationService.GetResourceAsync("Admin.System.Maintenance.BackupDatabase.DatabaseResited"));
                    }
                    break;
            }
        }
