...
public partial class MessageTemplateModelFactory : IMessageTemplateModelFactory
{
    ...
    protected readonly ISiteMappingSupportedModelFactory _storeMappingSupportedModelFactory;
    protected readonly ISiteService _storeService;
    ...
    public MessageTemplateModelFactory(
        ...
        ISiteMappingSupportedModelFactory storeMappingSupportedModelFactory,
    ...
    public virtual async Task<MessageTemplateSearchModel> PrepareMessageTemplateSearchModelAsync(MessageTemplateSearchModel searchModel)
    {
        ...
        await _baseAdminModelFactory.PrepareSitesAsync(searchModel.AvailableSites);
        ...
        searchModel.HideSitesList = searchModel.AvailableSites.SelectionIsNotPossible();
    ...
    public virtual async Task<MessageTemplateListModel> PrepareMessageTemplateListModelAsync(MessageTemplateSearchModel searchModel)
    {
        ...
        var messageTemplates = (await _messageTemplateService
            .GetAllMessageTemplatesAsync(searchModel.SearchSiteId, searchModel.SearchKeywords, isActive)).ToPagedList(searchModel);

        //prepare store names (to avoid loading for each message template)
        var stores = (await _storeService.GetAllSitesAsync()).Select(store => new { store.Id, store.Name }).ToList();
        ...
        var model = await new MessageTemplateListModel().PrepareToGridAsync(searchModel, messageTemplates, () =>
        {
            return messageTemplates.SelectAwait(async messageTemplate =>
            {
                ...
                if (messageTemplate.LimitedToSites)
                {
                    await _storeMappingSupportedModelFactory.PrepareModelSitesAsync(messageTemplateModel, messageTemplate, false);
                    var storeNames = stores
                        .Where(store => messageTemplateModel.SelectedSiteIds.Contains(store.Id)).Select(store => store.Name);
                    messageTemplateModel.ListOfSites = string.Join(", ", storeNames);
                }
                else
                {
                    var allstores = await _localizationService.GetResourceAsync("Admin.Configuration.Settings.AllSettings.Fields.SiteName.AllSites");
                    messageTemplateModel.ListOfSites = allstores;
                }
        ...
        MessageTemplate messageTemplate, bool excludeProperties = false)
    {
        ...
        //prepare available stores
        await _storeMappingSupportedModelFactory.PrepareModelSitesAsync(model, messageTemplate, excludeProperties);
