...
using AssetForge.Web.Framework.Security;
...
public partial class CommonModelFactory : ICommonModelFactory
{
    ...
    /// <summary>
    /// Prepare site URL warning model
    /// </summary>
    /// <param name="models">List of system warning models</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task PrepareSiteUrlWarningModelAsync(IList<SystemWarningModel> models)
    {
        ArgumentNullException.ThrowIfNull(models);

        //check whether current site URL matches the site configured URL
        var site = await _siteContext.GetCurrentSiteAsync();
        var currentSiteUrl = site.Url;
        if (!string.IsNullOrEmpty(currentSiteUrl) &&
            (currentSiteUrl.Equals(_webHelper.GetSiteLocation(false), StringComparison.InvariantCultureIgnoreCase) ||
             currentSiteUrl.Equals(_webHelper.GetSiteLocation(true), StringComparison.InvariantCultureIgnoreCase)))
        {
        ...
        models.Add(new SystemWarningModel
        {
            ...
            Text = string.Format(await _localizationService.GetResourceAsync("Admin.System.Warnings.URL.NoMatch"),
                currentSiteUrl, _webHelper.GetSiteLocation(false))
        });
    }
    ...
    protected virtual async Task PreparePluginsOverrideSameInterfaceWarningModelAsync(IList<SystemWarningModel> models)
    {
        ...
        var baseLibraries = new[] { "AssetForge.Core", "AssetForge.Data", "AssetForge.Services", "AssetForge.App", "AssetForge.App.Framework" };
        var overridenServices = _serviceCollection.Where(p =>
                p.ServiceType.FullName != null &&
                p.ServiceType.FullName.StartsWith("AssetForge.", StringComparison.InvariantCulture) &&
    ...
    public virtual async Task<SystemInfoModel> PrepareSystemInfoModelAsync(SystemInfoModel model)
    {
        ...
        model.AssetForgeVersion = AssetForgeVersion.FULL_VERSION;
    ...
    public virtual async Task<IList<SystemWarningModel>> PrepareSystemWarningModelsAsync()
    {
        ...
        //site URL
        await PrepareSiteUrlWarningModelAsync(models);
    ...
    public virtual Task<BackupFileListModel> PrepareBackupFileListModelAsync(BackupFileSearchModel searchModel)
    {
        ...
        var model = new BackupFileListModel().PrepareToGrid(searchModel, backupFiles, () =>
        {
            return backupFiles.Select(file => new BackupFileModel
            {
                ...
                Link = $"{_webHelper.GetSiteLocation()}db_backups/{_fileProvider.GetFileName(file)}"
            });
        });
            ...
            return urlRecords.SelectAwait(async urlRecord =>
            {
                ...
                switch (entityName)
                {
                    ...
                    case "newsitem":
                        ...
                        break;
                    case "page":
                        detailsUrl = urlHelper.Action("Edit", "Page", new { id = urlRecord.EntityId });
                        break;
                }
    ...
    public virtual async Task<LanguageSelectorModel> PrepareLanguageSelectorModelAsync()
    {
        ...
        var model = new LanguageSelectorModel
        {
            ...
            AvailableLanguages = (await _languageService
                    .GetAllLanguagesAsync(siteId: site.Id))
    ...
    //    var customerRoleIds = new[] { (await _customerService.GetCustomerRoleBySystemNameAsync(CustomerDefaults.RegisteredRoleName)).Id };
