...
using AssetForge.Web.Models.Install;
...
namespace AssetForge.App.Controllers
{
    public class InstallController : Controller
    {
        ...
        protected readonly Lazy<AssetForgeHttpClient> _ixHttpClient;
        ...
        public InstallController(AppSettings appSettings,
          ...
          Lazy<AssetForgeHttpClient> ixHttpClient)
        {
        ...
        public virtual async Task<IActionResult> Index(InstallModel model)
        {
            ...
            try
            {
                ...
                if (model.SubscribeNewsletters)
                {
                    try
                    {
                        var resultRequest = await _ixHttpClient.Value.SubscribeNewslettersAsync(model.AdminEmail);
                    }
                    catch
                    {
                        // ignored
                ...
                var cultureInfo = new CultureInfo(CommonDefaults.DefaultLanguageCulture);
                var regionInfo = new RegionInfo(CommonDefaults.DefaultLanguageCulture);
                ...
                if (model.InstallRegionalResources)
                {
                    ...
                    if (cultureInfo.Name != CommonDefaults.DefaultLanguageCulture)
                    {
                        try
                        {
                            var languageCode = _locService.Value.GetCurrentLanguage().Code[0..2];
                            var resultString = await _ixHttpClient.Value.InstallationCompletedAsync(model.AdminEmail, languageCode, cultureInfo.Name);
                            ...
                            if (result != null && result.LanguagePack.Progress > CommonDefaults.LanguagePackMinTranslationProgressToInstall)
                            {
                ...
                {
                    pluginsIgnoredDuringInstallation = _appSettings.Get<InstallationConfig>().DisabledPlugins.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(pluginName => pluginName.Trim()).ToList();
                }
        ...
        public virtual async Task<IActionResult> RestartInstallLanguage()
        {
            if (!DataSettingsManager.IsDatabaseInstalled())
                return RedirectToRoute("Homepage");
            ...
            await localizationService.DeleteLocaleResourcesAsync("", language.Id);
