...
public partial class CustomerController : BasePublicController
{
    ...
    protected virtual async Task<string> ParseSelectedProviderAsync(IFormCollection form)
    {
        ...
        var multiFactorAuthenticationProviders = await _multiFactorAuthenticationPluginManager.LoadActivePluginsAsync(await _workContext.GetCurrentCustomerAsync(), site.Id);
                ...
                case CustomerLoginResults.MultiFactorAuthenticationRequired:
                    {
                        ...
                        await HttpContext.Session.SetAsync(
                            CustomerDefaults.CustomerMultiFactorAuthenticationInfo,
    ...
    public virtual async Task<IActionResult> Logout()
    {
        ...
        if (_workContext.OriginalCustomerIfImpersonated != null)
        {
            ...
            await _customerActivityService.InsertActivityAsync(_workContext.OriginalCustomerIfImpersonated, "Impersonation.Finished",
                string.Format(await _localizationService.GetResourceAsync("ActivityLog.Impersonation.Finished.SiteOwner"),
            ...
            await _genericAttributeService
                .SaveAttributeAsync<int?>(_workContext.OriginalCustomerIfImpersonated, CustomerDefaults.ImpersonatedCustomerIdAttribute, null);
        ...
        //activity log
        await _customerActivityService.InsertActivityAsync(customer, "PublicSite.Logout",
            await _localizationService.GetResourceAsync("ActivityLog.PublicSite.Logout"), customer);
        ...
        if (_siteInformationSettings.DisplayEuCookieLawWarning)
        {
            ...
            //the only good solution in this case is to site a temporary variable
            //indicating that the EU cookie popup window should not be displayed on the next page open (after logout redirection to homepage)
            //but it'll be displayed for further page loads
            TempData[$"{CookieDefaults.Prefix}{CookieDefaults.IgnoreEuCookieLawWarning}"] = true;
        }
    ...
    //            await _genericAttributeService.SaveAttributeAsync(customer, CustomerDefaults.PasswordRecoveryTokenAttribute,
    //                passwordRecoveryToken.ToString());
    //            DateTime? generatedDateTime = DateTime.UtcNow;
    //            await _genericAttributeService.SaveAttributeAsync(customer,
    //                CustomerDefaults.PasswordRecoveryTokenDateGeneratedAttribute, generatedDateTime);
    ...
    //    if (string.IsNullOrEmpty(await _genericAttributeService.GetAttributeAsync<string>(customer, CustomerDefaults.PasswordRecoveryTokenAttribute)))
    //    {
    ...
    //                    await _workflowMessageService.SendNewVatSubmittedSiteOwnerNotificationAsync(customer, model.VatNumber, vatAddress, _localizationSettings.DefaultAdminLanguageId);
    //            }
    ...
    //                var newsletter = await _newsLetterSubscriptionService.GetNewsLetterSubscriptionByEmailAndSiteIdAsync(customerEmail, site.Id);
    //                if (newsletter != null)
    //                {
    //                    if (model.Newsletter)
    //                    {
    ...
    //                await _workflowMessageService.SendCustomerRegisteredSiteOwnerNotificationMessageAsync(customer,
    ...
    //                    //send VAT number admin notification
    //                    if (!string.IsNullOrEmpty(model.VatNumber) && _taxSettings.EuVatEmailAdminWhenNewVatSubmitted)
    //                        await _workflowMessageService.SendNewVatSubmittedSiteOwnerNotificationAsync(customer,
    ...
    //                await _genericAttributeService.SaveAttributeAsync(customer, CustomerDefaults.SignatureAttribute, model.Signature);
    ...
    //    var cToken = await _genericAttributeService.GetAttributeAsync<string>(customer, CustomerDefaults.EmailRevalidationTokenAttribute);
    //    if (string.IsNullOrEmpty(cToken))
    //    {
    ...
    //    await _customerService.UpdateCustomerAsync(customer);
    //    await _genericAttributeService.SaveAttributeAsync(customer, CustomerDefaults.EmailRevalidationTokenAttribute, "");
    ...
    //    var customAttributes = await _addressAttributeParser.ParseCustomAttributesAsync(form, AssetForgeCommonDefaults.AddressAttributeControlName);
    ...
    //        try
    //        {
    //            var customerAvatar = await _pictureService.GetPictureByIdAsync(await _genericAttributeService.GetAttributeAsync<int>(customer, CustomerDefaults.AvatarPictureIdAttribute));
    //            if (uploadedFile != null && !string.IsNullOrEmpty(uploadedFile.FileName))
    //            {
    //                var avatarMaxSize = _customerSettings.AvatarMaximumSizeBytes;
    //                if (uploadedFile.Length > avatarMaxSize)
    //                    throw new AssetForgeException(string.Format(await _localizationService.GetResourceAsync("Account.Avatar.MaximumUploadedFileSize"), avatarMaxSize));
    ...
    //            await _genericAttributeService.SaveAttributeAsync(customer, CustomerDefaults.AvatarPictureIdAttribute, customerAvatarId);

    //            model.AvatarUrl = await _pictureService.GetPictureUrlAsync(
    //                await _genericAttributeService.GetAttributeAsync<int>(customer, CustomerDefaults.AvatarPictureIdAttribute),
    ...
    //    await _genericAttributeService.SaveAttributeAsync(customer, CustomerDefaults.AvatarPictureIdAttribute, 0);
    ...
    //    //export
    //    var bytes = await _exportManager.ExportCustomerGdprInfoToXlsxAsync(customer, site.Id);
    ...
    //    await _workflowMessageService.SendDeleteCustomerRequestSiteOwnerNotificationAsync(customer, _localizationSettings.DefaultAdminLanguageId);
    ...
    //        {
    //            var remainingAmount = await _currencyService.ConvertFromPrimarySiteCurrencyAsync(await _giftCardService.GetGiftCardRemainingAmountAsync(giftCard), await _workContext.GetWorkingCurrencyAsync());
    ...
    //                {
    //                    await _genericAttributeService
    //                        .SaveAttributeAsync(customer, CustomerDefaults.SelectedMultiFactorAuthenticationProviderAttribute, string.Empty);
    ...
    //                var lastSavedProvider = await _genericAttributeService.GetAttributeAsync<string>(customer, CustomerDefaults.SelectedMultiFactorAuthenticationProviderAttribute);
    ...
    //                {
    //                    await _genericAttributeService.SaveAttributeAsync(customer, CustomerDefaults.SelectedMultiFactorAuthenticationProviderAttribute, selectedProvider);
