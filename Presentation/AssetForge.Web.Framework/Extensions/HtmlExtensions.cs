...
namespace AssetForge.Web.Framework.Extensions;
...
public static class HtmlExtensions
{
    ...
    /// <param name="ignoreIfSeveralSites">A value indicating whether to ignore localization if we have multiple sites</param>
    ...
    public static async Task<IHtmlContent> LocalizedEditorAsync<TModel, TLocalizedModelLocal>(this IHtmlHelper<TModel> helper,
        ...
        bool ignoreIfSeveralSites = false,
        ...
        where TLocalizedModelLocal : ILocalizedLocaleModel
    {
        ...
        if (ignoreIfSeveralSites)
        {
            var siteService = EngineContext.Current.Resolve<ISiteService>();

            if ((await siteService.GetAllSitesAsync()).Count >= 2)
    ...
    /// <summary>
    /// Gets a selected card name (used in admin area to site selected panel name)
    /// </summary>
    ...
    /// <summary>
    /// Gets a selected tab name (used in admin area to site selected tab name)
    /// </summary>
    /// <param name="helper">HtmlHelper</param>
