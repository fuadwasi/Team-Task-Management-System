...
public abstract partial class BaseController : Controller
{
    ...
    protected virtual async Task<string> RenderPartialViewToStringAsync(string viewName, object model)
    {
        ...
        //as we do in the /AssetForge.Web.Framework/Mvc/Filters/PublishModelEventsAttribute.cs for controllers
        switch (model)
        {
            case BaseModel ixModel:
                {
                    ...
                    eventPublisher.ModelPreparedAsync(ixModel).Wait();
                    break;
                }
            case IEnumerable<BaseModel> modelCollection:
                {
                    var eventPublisher = EngineContext.Current.Resolve<IEventPublisher>();
    ...
    /// <summary>
    /// Display "Edit" (manage) link (in public site)
    /// </summary>
    /// <param name="editPageUrl">Edit page URL</param>
    protected virtual void DisplayEditLink(string editPageUrl)
    {
        var ixHtmlHelper = EngineContext.Current.Resolve<UI.IAssetForgeHtmlHelper>();

        ixHtmlHelper.AddEditPageUrl(editPageUrl);
    }
    ...
    public virtual void SaveSelectedCardName(string cardName, bool persistForTheNextRequest = true)
    {
        ...
        //"GetSelectedCardName" method of \AssetForge.Web.Framework\Extensions\HtmlExtensions.cs
        ArgumentException.ThrowIfNullOrEmpty(cardName);

        const string dataKey = "ix.selected-card-name";
    ...
    {
        //keep this method synchronized with
        //"GetSelectedTabName" method of \AssetForge.Web.Framework\Extensions\HtmlExtensions.cs
        ...
        var dataKey = "ix.selected-tab-name";
