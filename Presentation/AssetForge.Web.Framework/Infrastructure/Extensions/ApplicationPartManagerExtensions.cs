...
public static partial class ApplicationPartManagerExtensions
{
    ...
    private static readonly IAssetForgeFileProvider _fileProvider;
    ...
    /// <param name="fileProvider">AssetForge file provider</param>
    /// <returns>Assembly</returns>
    private static Assembly PerformFileDeploy(this ApplicationPartManager applicationPartManager,
        string assemblyFile, PluginConfig pluginConfig, IAssetForgeFileProvider fileProvider)
    {
        //ensure for proper directory structure
        if (string.IsNullOrEmpty(assemblyFile) ||
            string.IsNullOrEmpty(fileProvider.GetParentDirectory(assemblyFile)))
            throw new InvalidOperationException(
                $"The plugin directory for the {fileProvider.GetFileName(assemblyFile)} file exists in a directory outside of the allowed ixCms directory hierarchy");
