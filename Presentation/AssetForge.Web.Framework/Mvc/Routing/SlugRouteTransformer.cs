...
using AssetForge.Web.Framework.Events;
...
public partial class SlugRouteTransformer : DynamicRouteValueTransformer
{
    ...
    protected virtual async Task SingleSlugRoutingAsync(HttpContext httpContext, RouteValueDictionary values, UrlRecord urlRecord, string catalogPath)
    {
        ...
        if (_localizationSettings.SeoFriendlyUrlsForLanguagesEnabled && values.TryGetValue(RoutingDefaults.RouteValue.Language, out var langValue))
        {
            var site = await _siteContext.GetCurrentSiteAsync();
            var languages = await _languageService.GetAllLanguagesAsync(siteId: site.Id);
        ...
        switch (urlRecord.EntityName)
        {
            ...
            //    RouteToAction(values, "Product", "ProductDetails", slug, (RoutingDefaults.RouteValue.ProductId, urlRecord.EntityId));
            //    return;

            //case var name when name.Equals(nameof(ProductTag), StringComparison.InvariantCultureIgnoreCase):
            //    RouteToAction(values, "Catalog", "ProductsByTag", slug, (RoutingDefaults.RouteValue.ProductTagId, urlRecord.EntityId));
            //    return;

            //case var name when name.Equals(nameof(Category), StringComparison.InvariantCultureIgnoreCase):
            //    RouteToAction(values, "Catalog", "Category", slug, (RoutingDefaults.RouteValue.CategoryId, urlRecord.EntityId));
            //    return;

            //case var name when name.Equals(nameof(NewsItem), StringComparison.InvariantCultureIgnoreCase):
            //    RouteToAction(values, "News", "NewsItem", slug, (RoutingDefaults.RouteValue.NewsItemId, urlRecord.EntityId));
            //    return;

            //case var name when name.Equals(nameof(BlogPost), StringComparison.InvariantCultureIgnoreCase):
            //    RouteToAction(values, "Blog", "BlogPost", slug, (RoutingDefaults.RouteValue.BlogPostId, urlRecord.EntityId));
            //    return;
    ...
    //        (RoutingDefaults.RouteValue.ProductId, urlRecord.EntityId), (RoutingDefaults.RouteValue.CatalogSeName, catalogSeName));
    //    return true;
    //}
    ...
    {
        values[RoutingDefaults.RouteValue.Controller] = "Common";
        values[RoutingDefaults.RouteValue.Action] = "InternalRedirect";
        values[RoutingDefaults.RouteValue.Url] = $"{httpContext.Request.PathBase}{path}{httpContext.Request.QueryString}";
        values[RoutingDefaults.RouteValue.PermanentRedirect] = permanent;
        httpContext.Items[HttpDefaults.GenericRouteInternalRedirect] = true;
    }
    ...
    {
        values[RoutingDefaults.RouteValue.Controller] = controller;
        values[RoutingDefaults.RouteValue.Action] = action;
        values[RoutingDefaults.RouteValue.SeName] = slug;
    ...
    public override async ValueTask<RouteValueDictionary> TransformAsync(HttpContext httpContext, RouteValueDictionary routeValues)
    {
        ...
        if (!values.TryGetValue(RoutingDefaults.RouteValue.SeName, out var slug))
            return values;
        ...
        var catalogPath = values.TryGetValue(RoutingDefaults.RouteValue.CatalogSeName, out var catalogPathValue)
