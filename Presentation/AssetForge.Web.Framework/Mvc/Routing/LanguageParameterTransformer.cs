...
public partial class LanguageParameterTransformer : IOutboundParameterTransformer
{
    ...
    public string TransformOutbound(object value)
    {
        ...
        if (routeValues.TryGetValue(RoutingDefaults.RouteValue.Language, out var routeValue))
        {
            //ensure this language is available
            var code = routeValue?.ToString();
            var siteContext = EngineContext.Current.Resolve<ISiteContext>();
            var site = siteContext.GetCurrentSite();
            var languages = _languageService.GetAllLanguages(siteId: site.Id);
