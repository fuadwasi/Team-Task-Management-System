...
namespace AssetForge.Web.Framework.Mvc.ModelBinding;

/// <summary>
/// Represents metadata provider that adds custom attributes to the model's metadata, so it can be retrieved later
/// </summary>
public partial class MetadataProvider : IDisplayMetadataProvider
{
    /// <summary>
    ...
    public void CreateDisplayMetadata(DisplayMetadataProviderContext context)
    {
        ...
        foreach (var additionalValue in additionalValues)
        {
            if (context.DisplayMetadata.AdditionalValues.ContainsKey(additionalValue.Name))
                throw new Core.AssetForgeException("There is already an attribute with the name '{0}' on this model", additionalValue.Name);
